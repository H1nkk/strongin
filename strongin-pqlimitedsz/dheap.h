...
public:
    dheaplimited_linear(size_t d, size_t sz = 0, size_t maxsz = _CRT_SIZE_MAX) : mem(sz), size(0), d(d), height(0), maxsz(maxsz) {
        if (d < 1) throw "d < 1";
    }
    void insert(T x) {
        if (size == maxsz) {
...
class dheaplimited_multimap {
    ...
    int minInd;
    T Tmin;
    multimap<T, size_t> mp; // map из элемента кучи в индекс
...
public:
    dheaplimited_multimap(size_t d, size_t sz = 0, size_t maxsz = _CRT_SIZE_MAX) : mem(sz), size(0), d(d), height(0), maxsz(maxsz) {
        if (d < 1) throw "d < 1";
    }
    void insert(T x) {
        if (size == maxsz) {
